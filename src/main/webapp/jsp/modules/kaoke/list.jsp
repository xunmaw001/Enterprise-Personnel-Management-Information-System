...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">考核管理</li>
                            <li class="breadcrumb-item active">考核列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="kaokeNameSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="考核名称" aria-controls="tableId">
                                        </div>
                                    </div>
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        考核类型
                                        <div class="layui-input-inline">
                                            <select name="kaokeTypesSelectSearch"  style="width: 150px;" id="kaokeTypesSelectSearch" class="form-control form-control-sm"
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        是否通过
                                        <div class="layui-input-inline">
                                            <select name="kaokeYesnoTypesSelectSearch"  style="width: 150px;" id="kaokeYesnoTypesSelectSearch" class="form-control form-control-sm"
                                                    aria-controls="tableId">
                                            </select>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        <!-- 本表的级联字段表 -->
        var kaokeTypesOptions = [];
        var kaokeYesnoTypesOptions = [];
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //考核名称
            var kaokeNameSearchInput = $('#kaokeNameSearch');
            if( kaokeNameSearchInput != null){
                if (kaokeNameSearchInput.val() != null && kaokeNameSearchInput.val() != '') {
                    searchForm.kaokeName = $('#kaokeNameSearch').val();
                }
            }
     
                //考核类型
            var kaokeTypesSelectSearchInput = document.getElementById("kaokeTypesSelectSearch");
            if(kaokeTypesSelectSearchInput != null){
                var kaokeTypesIndex = kaokeTypesSelectSearchInput.selectedIndex;
                if( kaokeTypesIndex  != 0){
                    searchForm.kaokeTypes= document.getElementById("kaokeTypesSelectSearch").options[kaokeTypesIndex].value;
                }
            }
         
                //是否通过
            var kaokeYesnoTypesSelectSearchInput = document.getElementById("kaokeYesnoTypesSelectSearch");
            if(kaokeYesnoTypesSelectSearchInput != null){
                var kaokeYesnoTypesIndex = kaokeYesnoTypesSelectSearchInput.selectedIndex;
                if( kaokeYesnoTypesIndex  != 0){
                    searchForm.kaokeYesnoTypes= document.getElementById("kaokeYesnoTypesSelectSearch").options[kaokeYesnoTypesIndex].value;
                }
            }
                    getDataList();
        }
        ...
        function getDataList() {
            http("kaoke/page", "GET", {
                ...
                order: sortOrder,
                //本表的
                kaokeName: searchForm.kaokeName,
                kaokeTypes: searchForm.kaokeTypes,
                kaokeYesnoTypes: searchForm.kaokeYesnoTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //考核名称
            var kaokeNameCell = document.createElement('td');
            kaokeNameCell.innerHTML = item.kaokeName;
            row.appendChild(kaokeNameCell);


            //考核类型
            var kaokeTypesCell = document.createElement('td');
            kaokeTypesCell.innerHTML = item.kaokeValue;
            row.appendChild(kaokeTypesCell);


                //考核详情
            var kaokeContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.kaokeContent && item.kaokeContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.kaokeContent.indexOf('<img');
                var endIndex = item.kaokeContent.indexOf('>');
                    kaokeContentCell.innerHTML = item.kaokeContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.kaokeContent != null && item.kaokeContent != "" && item.kaokeContent.length >= 11) {
                        kaokeContentCell.innerHTML = item.kaokeContent.substring(0, 10) + "...";
                } else {
                        kaokeContentCell.innerHTML = item.kaokeContent;
                }
            }
            row.appendChild(kaokeContentCell);


            //是否通过
            var kaokeYesnoTypesCell = document.createElement('td');
            kaokeYesnoTypesCell.innerHTML = item.kaokeYesnoValue;
            row.appendChild(kaokeYesnoTypesCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("kaoke/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addkaoke", "addkaoke");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function kaokeTypesSelectSearch() {
            var kaokeTypesSelectSearch = document.getElementById('kaokeTypesSelectSearch');
            if(kaokeTypesSelectSearch != null) {
                kaokeTypesSelectSearch.add(new Option('-请选择-',''));
                if (kaokeTypesOptions != null && kaokeTypesOptions.length > 0){
                    for (var i = 0; i < kaokeTypesOptions.length; i++) {
                            kaokeTypesSelectSearch.add(new Option(kaokeTypesOptions[i].indexName,kaokeTypesOptions[i].codeIndex));
                    }
                }
            }
        }
         
        function kaokeYesnoTypesSelectSearch() {
            var kaokeYesnoTypesSelectSearch = document.getElementById('kaokeYesnoTypesSelectSearch');
            if(kaokeYesnoTypesSelectSearch != null) {
                kaokeYesnoTypesSelectSearch.add(new Option('-请选择-',''));
                if (kaokeYesnoTypesOptions != null && kaokeYesnoTypesOptions.length > 0){
                    for (var i = 0; i < kaokeYesnoTypesOptions.length; i++) {
                            kaokeYesnoTypesSelectSearch.add(new Option(kaokeYesnoTypesOptions[i].indexName,kaokeYesnoTypesOptions[i].codeIndex));
                    }
                }
            }
        }
    ...
    //查询当前表搜索条件所有列表
            function kaokeTypesSelect() {
                //填充下拉框选项
                http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kaoke_types", "GET", {}, (res) => {
                    if(res.code == 0){
                        kaokeTypesOptions = res.data.list;
                    }
                });
            }
            function kaokeYesnoTypesSelect() {
                //填充下拉框选项
                http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kaoke_yesno_types", "GET", {}, (res) => {
                    if(res.code == 0){
                        kaokeYesnoTypesOptions = res.data.list;
                    }
                });
            }
        ...
        //当前表的下拉框赋值
            ...
            kaokeTypesSelectSearch();
                         
            kaokeYesnoTypesSelectSearch();
