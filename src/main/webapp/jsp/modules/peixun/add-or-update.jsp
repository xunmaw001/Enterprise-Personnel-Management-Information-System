                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group  col-md-6">
                                        <label>培训内容</label>
                                        <input id="peixunContentupload" name="file" type="file">
                                        <script id="peixunContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var peixunContentUe = UE.getEditor('peixunContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="peixunContent" id="peixunContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#peixunContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('peixunContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.peixunContent != null && ruleForm.peixunContent != 'null' && ruleForm.peixunContent != '' && $("#peixunContentupload").length>0) {

            var peixunContentUeditor = UE.getEditor('peixunContentEditor');
            peixunContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.peixunContent != null){
                    mes = ''+ ruleForm.peixunContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                peixunContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#peixunContentupload").length>0) {
            var peixunContentEditor = UE.getEditor('peixunContentEditor');
            if (peixunContentEditor.hasContents()) {
                $('#peixunContent-input').attr('value', peixunContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#peixunContent").val(ruleForm.peixunContent);
